...
| 2 | 機能完成度 | 62 | 63 | DB ヘルス API・セッション整理を追加 |
| 3 | UI/UX | 68 | 68 | 変更なし |
...
| 10 | セキュリティ | 73 | 76 | Referrer/Permissions-Policy 追加 |
| 11 | 可用性・バックアップ | 45 | 55 | バックアップ/復旧 runbook 追加 |
| 12 | 監視・障害対応 | 30 | 38 | 監視 runbook＋DB ヘルス追加（アラートは未実装） |
| 13 | テスト | 48 | 70 | 155 テスト（DB 4 スイート含む）を CI で実行・カバレッジ 69% |
...
### テスト
- 新規 49 件（計 155 件）: 401 warning、検証整合警告、セキュリティヘッダー、DB ヘルス、スコアリング分解、JSON→DB 行変換
- DB 依存 4 スイート（Phase A/B/C・ローカル認証）をローカル PostGIS で実行し全成功（migration 5 本→seed→test）
- カバレッジ 34% → 69%（web/api_v1 92%、web/auth 91%、db 100%）
...
| 優先度 | 課題 | 状態 |
|---|---|---|
| P0 | アラート基盤（検証失敗・ヘルス異常の通知） | 未実装（runbook のみ） |
| P0 | ロール即時失効（#61） | 未実装 |
| P1 | レート制限 | 未実装 |
| P1 | E2E（Playwright） | 未実装 |
| P1 | AD-6 監査ログ DB ロール強制 | 未適用（DB 正本切替時に実施） |
| P1 | Try-it console（#66）・OpenAPI import（#65） | 未実装 |
| P2 | モバイル/PWA・オフライン | 未実装 |
