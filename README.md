...
| 区分                  | 内容                                                                                                                                                                                                                                                                                                    |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 📚 台帳カバレッジ     | 登録50件のうち接続検証済は7件、本格利用候補は3件のみ。多くのデータは今後の検証待ち                                                                                                                                                                                                                      |
| 🔑 認証・アクセス制御 | 静的 Web UI（`web/server.py`）自体はログイン機構を持たず、社内 LAN + 外部公開時は Cloudflare Access（One-Time PIN）で保護する。新しい API v1 レイヤ（`web/api_v1.py`・opt-in）は Entra ID OIDC + 5 ロール RBAC を実装済みで、書込系（登録・更新・論理削除）は認証必須（詳細: `docs/entra-id-setup.md`） |
